...
class Day1FrameworkTest(unittest.TestCase):
    ...
    def test_team_event_can_be_saved(self) -> None:
        ...
        self.assertEqual(record.event_type, "phone_usage")
    ...
    def test_query_finds_phone_usage(self) -> None:
        events = [
            TeamEvent("reading", 12.0, 18.0, 1, 0.94, "学生阅读书本"),
            TeamEvent("writing", 120.5, 180.0, 1, 0.89, "学生拿起笔书写"),
            TeamEvent("phone_usage", 632.5, 640.0, 1, 0.91, "学生暂停学习并使用手机"),
        ]
        for event in events:
            self.service.remember_event(event)

        result = self.service.search("什么时候使用了手机？", top_k=1)[0]
        self.assertEqual(result.record.event_type, "phone_usage")
        self.assertEqual(result.record.timestamp, 632.5)

    def test_invalid_event_time_is_rejected(self) -> None:
        event = TeamEvent("phone_usage", 20.0, 10.0, 1, 0.91, "学生使用手机")
