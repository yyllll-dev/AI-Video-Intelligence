...
请严格按时间顺序复述上面的已确认事件：
- 只描述人物做了什么以及动作先后，不扩写、不润色、不解释原因。
- 只能使用上面明确提供的事实，不得虚构或增加人物身份、物品、姿势、次数、情绪、兴趣、态度、习惯或学习效果。
- 禁止使用“可能、似乎、也许、或许、大概、推测、看起来、显示出、表明、体现”等推测或评价词。
- 不评价行为好坏，不提模型、检测、事件标签、置信度或“根据记录”。
- 必须合并连续重复事件；同一类“准备、整理或动作切换”只能概括一次，不得逐条重复相同句式。
- 内容尽量简短；事件很少时可以只写一两句话。
...
- reading：持续注视书本或文字资料；只要画面是在翻阅书籍、翻动书页、翻到下一页或换页，均视为阅读。刚拿出书、打开书、寻找页码或翻到指定目标页，但尚未持续阅读，不算阅读。
...
翻阅书籍、翻页、翻动书页、换页或翻到下一页一律选 reading，不选 other_behavior。只有明确寻找页码或翻到指定目标页才算准备。只有真正落笔才选 writing。
...
reading：眼睛持续看印刷书页；翻阅书籍、翻页、翻动书页、换页或翻到下一页仍是阅读。
...
other_behavior：拿出、移动、收起、整理物品，寻找页码或翻到指定目标页等准备/切换动作，动作混合或无法确认；正常翻页不属于 other_behavior。
